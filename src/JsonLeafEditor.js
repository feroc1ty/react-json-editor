...
var JsonLeafEditor = React.createClass({
    ...
    render: function () {
        ...
        var leaf = (this.state.editing
            ? [(<input value={this.state.jsValue} onChange={this.onChange} style={{background: 'transparent'}}/>),
            (<button onClick={this.commit} disabled={!this.isValid()}>commit</button>)]
            : [(<code className="editButton" onClick={this.edit}>{this.state.jsValue}</code>)]);
